...
public class AprioriUtil {
	/**
	 * 获得itemList的所有比其元素个数少一的子集; itemList的元素个数必须大于等于2
	 * 
	 * @param itemList
	 * @return
	 */
	...
	/**
	 * 判断一个itemList是不是真的候选频繁项目集， 只要有一个比它少一个元素的子集不是k-1频繁项 目集集合的元素，就被排除；因为，如果一个项目
	 * 集是频繁项目集的话，它的任何非空子集也都是 频繁项目集
	 * 
	 * @param foreFrequentItemsets
	 * @param itemList
	 * @return
	 */
	public static boolean isRealCandidateFrequentItemset(List<FrequentItemset> foreFrequentItemsets,
	 ...
	 * @return
	 */
	public static boolean isRealCandidateConsequent(List<List<String>> foreConsequents, List<String> itemList) {
		List<List<String>> subsetList = AprioriUtil.getSubsetList(itemList);
		for (List<String> subset : subsetList) {
	 ...
	 * @return
	 */
	public static List<String> getCondition(List<String> frequentItemList, List<String> consequent) {
	...
	/**
	 * 一对的意思就是，它们两长度一样，并且只有最后一 个元素不一样
	 * 
	 * @param list1
	 * @param list2
	 * @return
	 ...
	 * @return
	 */
	public static List<String> mergePairList(List<String> list1, List<String> list2) {
		List<String> afterMerge = new ArrayList<>();
		int listSize = list1.size();
		if (list1.get(listSize - 1).compareTo(list2.get(listSize - 1)) < 0) {
