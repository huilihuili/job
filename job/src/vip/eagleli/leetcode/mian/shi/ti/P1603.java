...
public class P1603 {
	public double[] intersection(int[] start1, int[] end1, int[] start2, int[] end2) {
		...
		if ((y4 - y3) * (x2 - x1) == (y2 - y1) * (x4 - x3)) {
			// 若平行，则判断 (x4, y4) 是否在「直线」(x1, y1)~(x2, y2) 上
			// 任意选择一个点，看这个点是否在另一条线上即可
			if ((y2 - y1) * (x4 - x1) == (y4 - y1) * (x2 - x1)) {

				// 判断 (x3, y3) 是否在「线段」(x1, y1)~(x2, y2) 上
				if (inside(x1, y1, x2, y2, x3, y3)) {
					hasAnswer = update(ans, x3, y3, hasAnswer);
				}

				// 判断 (x4, y4) 是否在「线段」(x1, y1)~(x2, y2) 上
				if (inside(x1, y1, x2, y2, x4, y4)) {
					hasAnswer = update(ans, x4, y4, hasAnswer);
				}

				// 判断 (x1, y1) 是否在「线段」(x3, y3)~(x4, y4) 上
				if (inside(x3, y3, x4, y4, x1, y1)) {
					hasAnswer = update(ans, x1, y1, hasAnswer);
				}

				// 判断 (x2, y2) 是否在「线段」(x3, y3)~(x4, y4) 上
				if (inside(x3, y3, x4, y4, x2, y2)) {
					hasAnswer = update(ans, x2, y2, hasAnswer);
				}
			}
		} else {
			// 联立方程得到 t1 和 t2 的值
	...
	public double[] intersection_(int[] start1, int[] end1, int[] start2, int[] end2) {
		int x1 = start1[0], y1 = start1[1];
		int x2 = end1[0], y2 = end1[1];
		int x3 = start2[0], y3 = start2[1];
		int x4 = end2[0], y4 = end2[1];
